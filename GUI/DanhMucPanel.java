...
public class DanhMucPanel extends JPanel {
    ...
    private JTextField txtSearch, txtMa, txtTen;
    private JButton btnReset, btnAdd, btnDelete;

    private List<DanhMucDTO> allList = new ArrayList<>();
    private List<DanhMucDTO> currentList = new ArrayList<>();
    ...
    private void initUI() {
        ...
        txtMa = new JTextField(15); txtTen = new JTextField(25);
        
        pnlRow2.add(createFilterField("Mã Danh Mục", txtMa, Color.WHITE, Color.decode("#DCDDE1")));
        pnlRow2.add(createFilterField("Tên Danh Mục", txtTen, Color.WHITE, Color.decode("#DCDDE1")));
        ...
        DefaultTableCellRenderer customRenderer = new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                ...
                else if (column == 1) { setHorizontalAlignment(SwingConstants.LEFT); setFont(new Font("Segoe UI", Font.BOLD, 15)); } 
        ...
        btnAdd = createButton("THÊM DANH MỤC", new Color(40, 167, 69));
        btnDelete = createButton("XÓA DANH MỤC", new Color(220, 53, 69));
    ...
    private JButton createButton(String text, Color bg) {
        ...
        btn.setBorderPainted(false); btn.setFocusPainted(false); btn.setPreferredSize(new Dimension(170, 42)); btn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        return btn;
    }

    private void loadTable(){ allList = bus.getAllDanhMuc(); showTable(allList); }
    
    private void showTable(List<DanhMucDTO> list){
        currentList = list; model.setRowCount(0);
        for(DanhMucDTO dm : list) model.addRow(new Object[]{ dm.getMaDanhMuc(), dm.getTenDanhMuc() });
    }
    ...
    private void initEvents(){
        ...
        // CHỨC NĂNG THÊM MỚI BẰNG HỘP THOẠI NHANH
        btnAdd.addActionListener(e -> {
            String input = JOptionPane.showInputDialog(this, "Nhập tên danh mục mới:", "Thêm Danh Mục", JOptionPane.PLAIN_MESSAGE);
            if(input != null && !input.trim().isEmpty()) {
                DanhMucDTO dm = new DanhMucDTO();
                dm.setTenDanhMuc(input.trim());
                if(bus.addDanhMuc(dm)) {
        ...
        btnDelete.addActionListener(e -> {
            ...
            if (row == -1) { JOptionPane.showMessageDialog(this, "Vui lòng chọn một danh mục để xóa!", "Thông báo", JOptionPane.WARNING_MESSAGE); return; }
            int confirm = JOptionPane.showConfirmDialog(this, "Xác nhận xóa danh mục này?", "Xác nhận xóa", JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                boolean ok = bus.deleteDanhMuc(currentList.get(row).getMaDanhMuc());
                if(ok) {
                    JOptionPane.showMessageDialog(this, "Xóa thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE); loadTable();
                } else {
                    JOptionPane.showMessageDialog(this, "Xóa thất bại! Danh mục đang liên kết với dữ liệu khác.", "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
        
        // CHỨC NĂNG SỬA BẰNG HỘP THOẠI NHANH
        table.addMouseListener(new MouseAdapter(){
            public void mouseClicked(MouseEvent e){
                if(e.getClickCount() == 2){
                    int row = table.getSelectedRow(); if(row == -1) return;
                    DanhMucDTO dm = currentList.get(row);
                    
                    String input = (String) JOptionPane.showInputDialog(
                            DanhMucPanel.this, "Sửa tên danh mục:", "Cập Nhật",
                            JOptionPane.PLAIN_MESSAGE, null, null, dm.getTenDanhMuc());
                            
                    if(input != null && !input.trim().isEmpty() && !input.equals(dm.getTenDanhMuc())) {
                        dm.setTenDanhMuc(input.trim());
                        if(bus.updateDanhMuc(dm)) {
                            JOptionPane.showMessageDialog(DanhMucPanel.this, "Cập nhật thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                            loadTable();
                        } else {
                            JOptionPane.showMessageDialog(DanhMucPanel.this, "Cập nhật thất bại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
            }
    ...
    private void searchAdvanced(){
        ...
        List<DanhMucDTO> filtered = new ArrayList<>();
        for(DanhMucDTO dm : allList){
            boolean match = true; 
            
            if(!key.isEmpty()) {
                if(!(dm.getMaDanhMuc().toLowerCase().contains(key) || dm.getTenDanhMuc().toLowerCase().contains(key))) match = false;
            }
            if(!ma.isEmpty() && !dm.getMaDanhMuc().toLowerCase().contains(ma)) match = false;
            if(!ten.isEmpty() && !dm.getTenDanhMuc().toLowerCase().contains(ten)) match = false;
            
            if(match) filtered.add(dm);
        }
        showTable(filtered);
    }

    private void resetFilter(){
        txtSearch.setText(""); txtMa.setText(""); txtTen.setText(""); loadTable();
    }
}